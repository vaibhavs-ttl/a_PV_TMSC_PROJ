...
public class MaintaintipsListFragment extends Fragment {
	...
	private String SpinerSelected;
	private Spinner spinner;

	private String[] taskname = { "PRECAUTION", "STEERING", "BRAKES", "BATTERY",
	...
	private int arr_images[] = { R.drawable.whiteprecautions, R.drawable.whitesteering,
	...
	private ListView lv, dialoglist, enginedialoglist;
	private TextView dialogtitle, enginedialogtitle;
	private ImageView dialogimg, dialogimgclose, enginedialogimg, enginedialogimgclose,
			enginedialogimgg;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		enginedialoglist.setOnTouchListener(new OnTouchListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (SpinerSelected.equals("ALERTS & INDICATIONS")) {
					...
					if (position == 0) {
						...
						/*
						 * } else if (position == 1) {
						 * dialogtitle.setText(parent
						 * .getItemAtPosition(position) .toString());
						 * 
						 * set_to_dialogadapter(MaintaintanceList_StringArray.dont_s
						 * ); dialog.show(); } else if (position == 2) {
						 * dialogtitle
						 * .setText(parent.getItemAtPosition(position)
						 * .toString());
						 * 
						 * set_to_dialogadapter(MaintaintanceList_StringArray.
						 * in_extended_use); dialog.show();
						 */
					}
				...
				if (event.getAction() == KeyEvent.ACTION_DOWN) {
					if (keyCode == KeyEvent.KEYCODE_BACK) {
						...
						tx.replace(R.id.frame_container,
								new MaintenanceTipsFragment()).addToBackStack(null).commit();
						return true;
					}
				}
				return false;
